...
class MainFrame:
    def __init__(self,janela,open_newprofile_window,delete_profile,update_profile):
        ...
        self.title = tk.Label(self.frame,text="Bem vindo ao catálogo de profissionais!")
        self.details = tk.Label(self.frame,text="Para Saber mais, clique 2 vezes no perfil desejado.")
    ...
    def build_window(self,display_infoframe):
        self.title.pack(pady=10)
        self.tabela.pack(pady=10)
        self.tabela.bind("<Double-1>",display_infoframe)
...
class FillInfoFrame:
    def __init__(self,janela,update_main_window,confirm_func,text_confirmbutton,text_mainlabel):
        self.frame = tk.Frame(janela)
        self.title = tk.Label(self.frame,text=text_mainlabel)
...
class InfoFrame:
    def __init__(self,janela,update_main_window):
        self.frame = tk.Frame(janela)
        self.title = tk.Label(self.frame,text="Detalhes do Profissional")
        self.personal_infos = [tk.Label(self.frame,text="") for _ in range(5)]
        self.adress_infos = [tk.Label(self.frame,text="")for _ in range(7)]
        self.returnbutton = tk.Button(self.frame,text="Retornar",command=update_main_window)

    def update_window(self,profile):

        self.personal_infos[0].config(text=f"Id:{profile.id}")
        self.personal_infos[1].config(text=f"Nome:{profile.nome}")
        self.personal_infos[2].config(text=f"CPF/CNPJ:{profile.cpf_cnpj}")
        self.personal_infos[3].config(text=f"Data de Nascimento:{profile.nascimento}")
        self.personal_infos[4].config(text=f"Contato:{profile.contato}")

        self.adress_infos[0].config(text=f"CEP:{profile.CEP}")
        self.adress_infos[1].config(text=f"Rua:{profile.rua}")
        self.adress_infos[2].config(text=f"Numero:{profile.numero}")
        self.adress_infos[3].config(text=f"Complemento:{profile.complemento}")
        self.adress_infos[4].config(text=f"Bairro:{profile.bairro}")
        self.adress_infos[5].config(text=f"Cidade:{profile.cidade}")
        self.adress_infos[6].config(text=f"UF:{profile.UF}")

    def build_window(self):
        self.title.grid(row=0,columnspan=2,pady=10)

        for i in range(len(self.personal_infos)):
            self.personal_infos[i].grid(row=i+1,column=1,pady=10)

        for i in range(len(self.adress_infos)):
            self.adress_infos[i].grid(row=i+1,column=2,pady=10)

        self.returnbutton.grid(row = 8, columnspan=2,pady=10)